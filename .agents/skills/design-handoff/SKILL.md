...
1. visual reference review, rendered critic, AI-TELL audit, visual production value audit와 visual fidelity review가 PASS이고 blocking issue가 해결됐는지 확인한다. `VISUAL_REFERENCE_TOOL_UNAVAILABLE`, `DESIGN_REVIEW_BLOCKED`나 3회 초과를 우회하지 않는다.
...
5. `DESIGN_AND_PUBLISH`면 `--for-publishing` gate도 확인하고 desktop review를 primary visual reference로 기존 `reference-publish`에 전달한다. tablet/mobile, design plan, visual direction과 section reference manifest는 responsive/visual intent다.
