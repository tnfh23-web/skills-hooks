...
`contract`는 `verification.sampleSelector`처럼 결정적 관찰 지점이 있어야 자동 실행한다는 뜻이다. 계약이 없으면 Motion QA는 PASS 대신 `DEFERRED`를 기록한다. Interaction QA는 Motion/Advanced 후보를 generic click으로 검사하지 않는다.
