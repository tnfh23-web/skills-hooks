...
import { AUTHORING_POLICIES, validateAuthoring } from '../tools/interaction-authoring.mjs';
...
try {
  ...
  assert.equal(discovered.interactionLanguage.version, 1);
  assert.equal(discovered.interactionLanguage.source, 'deterministic-dom-analysis');
  assert.equal(validateAuthoring(discovered).valid, true);
  assert.ok(discovered.candidates.every((item) => item.authoring?.policy));
  assert.ok(discovered.candidates.some((item) => item.authoring.policy === AUTHORING_POLICIES.AFFORDANCE_DRIVEN));
  assert.ok(discovered.candidates.some((item) => item.authoring.policy === AUTHORING_POLICIES.ENHANCED_MOTION));
  const discoveredMarquee = discovered.candidates.find((item) => item.recipe === 'marquee');
  assert.equal(discoveredMarquee.authoring.evidenceGate.passed, true, 'explicit marquee structure is sufficient authoring evidence');
  const malformedAuthoring = structuredClone(discovered);
  malformedAuthoring.candidates.find((item) => item.authoring.policy === AUTHORING_POLICIES.REQUIRED_BASELINE).authoring.requiredFeedback = ['hover'];
  assert.equal(validateInteractionPlan(malformedAuthoring).valid, false, 'authoring policy must reject incomplete baseline feedback');
