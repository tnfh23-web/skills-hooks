...
function choosePrimitive(pool, semanticType, interactionLanguage, motionLanguage) {
  ...
  const motionPreferred = Array.isArray(motionLanguage?.preferredPrimitives) ? motionLanguage.preferredPrimitives : [];
  const interactionPreferred = preferredPrimitives(semanticType, interactionLanguage);
  return motionPreferred.find((primitive) => allowed.includes(primitive))
    || interactionPreferred.find((primitive) => allowed.includes(primitive))
    || allowed[0]
    || null;
}
...
export function buildInteractionComposition({ candidates = [], actionableAuthoring = [], interactionLanguage = null, motionLanguage = null } = {}) {
  ...
  if (motionLanguage && /minimal|restricted|limited|none/i.test(motionLanguage.continuousMotionUsage || '') && continuous.length > 0) restraint.push('continuous-motion-restricted');
...
export function validateInteractionDiversity(plan) {
  ...
  const elementCount = (group) => Math.max(group.elementCount || 0, group.selectors?.length || 0, group.elements?.length || 0);
  ...
  for (const group of groups) { const count = elementCount(group); totalElements += count; primitiveCounts.set(group.primitive, (primitiveCounts.get(group.primitive) || 0) + count); if (group.primitive === 'opacity-shift' || group.primitive === 'all-elements-opacity-only') opacityElements += count; }
  ...
  const imageGroups = groups.filter((group) => ['CARD_LINK', 'IMAGE_CARD'].includes(group.semanticType)); const imageElements = imageGroups.reduce((total, group) => total + elementCount(group), 0); const scaledImageElements = imageGroups.filter((group) => /(?:global-scale|restrained-image-scale|scale(?:\(|-|$))/i.test(group.primitive || '')).reduce((total, group) => total + elementCount(group), 0); if (imageElements >= 3 && scaledImageElements / imageElements >= 0.8) errors.push('Anti-generic interaction failure: generic scale is repeated across at least 80% of image/card actionable elements.');
...
export function validateAuthoring(plan) {
  ...
  const hasLatestAuthoring = Boolean(plan?.interactionLanguage || plan?.actionableAuthoring || (plan?.candidates || []).some((candidate) => candidate?.authoring));
  if (hasLatestAuthoring && !plan?.interactionComposition) errors.push('interactionComposition is required for authored interaction plans');
  if (plan?.interactionComposition) { const roles = ['primary', 'secondary', 'continuous', 'restraint']; for (const role of roles) if (!Array.isArray(plan.interactionComposition[role])) errors.push(`interactionComposition.${role} must be an array`); const completeRoles = roles.every((role) => Array.isArray(plan.interactionComposition[role])); if (completeRoles) { const stateful = (plan.candidates || []).some((candidate) => ['tabs', 'accordion', 'dropdown', 'drawer', 'menu-state', 'carousel-state', 'scene-transition'].includes(candidate.recipe) && candidate.implementation !== 'skip') || (plan.actionableAuthoring || []).some((group) => ['TAB', 'STATE_NAV', 'CAROUSEL_CONTROL', 'MENU_CONTROL', 'SEARCH_CONTROL'].includes(group.semanticType)); if (stateful && plan.interactionComposition.primary.length === 0) errors.push('Stateful affordances require at least one primary interaction.'); if (plan.interactionComposition.continuous.length > 0 && plan.interactionComposition.primary.length === 0) errors.push('Continuous motion cannot be the only page-level interaction.'); } }
